...
##################################################
## simulating data for Figure 14 (right)
##################################################
...
arbvar <- FALSE
interv.vec <- seq(0,.5,length.out = 6)

set.frame <- var.frame <- NULL

############################### ICPH ###############################
s <- 0
for(k in 1:length(interv.vec)){
  interv <- interv.vec[k]
  for(i in 1:num.exp){
    print(paste("ICPH, intervention = ", interv, ", sim = ", i, "seed = ", s+1))
    s <- s+1
    set.seed(s)
    ...
    Y <- numeric(n)
    Y[E==1 & H==1] <- muY[1] + cbind(X1,X2)[E==1 & H==1,] %*% betaY[,1,drop=F] + sqrt(sigmaY)[1] * rnorm(sum(E==1 & H==1))
    Y[E==1 & H==2] <- muY[2] + cbind(X1,X2)[E==1 & H==2,] %*% betaY[,2,drop=F] + sqrt(sigmaY)[2] * rnorm(sum(E==1 & H==2))
    Y[E==2 & H==1] <- muY[1] + cbind(X1,X2)[E==2 & H==1,] %*% (betaY[,1,drop=F] + matrix(interv,2,1)) + sqrt(sigmaY)[1] * rnorm(sum(E==2 & H==1))
    Y[E==2 & H==2] <- muY[2] + cbind(X1,X2)[E==2 & H==2,] %*% betaY[,2,drop=F] + sqrt(sigmaY)[2] * rnorm(sum(E==2 & H==2))
    Y[E==3 & H==1] <- muY[1] + cbind(X1,X2)[E==3 & H==1,] %*% (betaY[,1,drop=F] + matrix(-interv,2,1)) + sqrt(sigmaY)[1] * rnorm(sum(E==3 & H==1))
    Y[E==3 & H==2] <- muY[2] + cbind(X1,X2)[E==3 & H==2,] %*% betaY[,2,drop=F] + sqrt(sigmaY)[2] * rnorm(sum(E==3 & H==2))
    ...
    tmp <- tryCatch({
      ...
      data.frame(n=n, seed=s, var=1:3, method="ICPH", intervention = interv, reject.nonCausal=NA)
    })
...
write.table(var.frame, "fig14_3_data.txt", quote = FALSE)
