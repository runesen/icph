...
# PC algorithm WITH BACKGROUND KNOWLEDGE -- NOT DONE
s <- 0
for(k in 1:length(n.vec)){
  n <- n.vec[k]
  for(i in 1:num.exp){
    ...
    tmp <- tryCatch({
      ...
      amat <- as(pc.fit, "amat")
      amat <- matrix(0,5,5)
      colnames(amat) <- rownames(amat)  <- c("X1", "X2", "X3", "Y", "E")
      amat[3:4,5] <-  amat[2,3] <- amat[5,3] <- amat[3,2] <- 1
      amat
      #addBgKnowledge(gInput=amat, x="E", y="X3")
      addBgKnowledge(gInput=amat, x="E", y="X3")
      amat <- as.matrix(amat)
      amat <- addBgKnowledge(gInput=amat, x="E", y="X1")
      amat <- addBgKnowledge(gInput=amat, x=rep("E", 4), y=c("X1", "X2", "X3", "Y"))
