...
##################################################
## simulating data for Figure 13 (right)
## the data for Figure 13 (left) are generated by 
## fig8_sim.R
##################################################
onServer <- TRUE
library(pracma)
...
for(method in method.vec){
  for(constraint in constraint.vec){
    ...
    for(k in 1:length(n.vec)){
        ...
        for(i in 1:num.exp){
          ...
          tmp <- tryCatch({
            result.icp <- icph(D = cbind(X1, X2, X3, Y), E = E, target = 4, model = "IID",
                               par.icp = list(output.pvalues = FALSE, silent=TRUE, stopIfEmpty = FALSE),
                               par.test = par.test)
      ...
      write.table(set.frame, "fig13_data_set.txt", quote = FALSE, sep="\t")
      write.table(var.frame, "fig13_data_var.txt", quote = FALSE)
    }
  }
}
