...
for (j in 1:12){
  aparfile[j]<-paste('data_fig11_fig12/Monthly_APAR_hd/apar.',201000+j,'.hd.SG.tif',sep='')
  swfile[j]<-paste("data_fig11_fig12/SW/GLDAS_NOAH025_M.A2010", mon[j],".021.nc4",sep="")
}

vpmfile <- list.files('data_fig11_fig12/MPI_HD/',pattern='tif',full.name=TRUE)

SIFfile<-list.files('data_fig11_fig12/monthly_SIF/',pattern='tif',full.name=TRUE)

lc<-raster('data_fig11_fig12/max.2010.hd.tif')
pct<-raster('data_fig11_fig12/max_per.2010.hd.tif')
lc[pct<0.75] <- NA
...
pdf("~/Google Drive/phd/causing/drafts/icph/figures/fig11.pdf", width = 12, height = 4)
