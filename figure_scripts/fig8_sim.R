  ...
  for(constraint in constraint.vec){
    ...
    for(k in 1:length(n.vec)){
      ...
      for(j in 1:length(dist.vec)){
        dist <- dist.vec[j]
        for(i in 1:num.exp){
          print(paste0(method, ", ",  constraint, ", n = ", n, ", s = ", s))
          s <- s+1
          set.seed(s)
          ...
          tmp <- tryCatch({
            result.icp <- icph(D = cbind(X1, X2, X3, Y), E = E, target = 4, model = "IID",
                               par.icp = list(output.pvalues = FALSE, silent =FALSE, stopIfEmpty = FALSE),
                               par.test = par.test)
      ...
      write.table(set.frame, "fig8_data_set.txt", quote = FALSE, sep="\t")
      write.table(var.frame, "fig8_data_var.txt", quote = FALSE)
    }
  }
}
