...
##################################################
## producing Figure 6 
## run script fig6_sim.R to generate 
## fig6_data_set.txt and fig6_data_set.txt
##################################################
...
## loading data
set.frame <- read.table("fig6_data_set.txt", header = TRUE, sep = "\t")
var.frame <- read.table("fig6_data_var.txt", header = TRUE)
...
pS <- ggplot(typeIS, aes(xmin=n-50, xmax = n+50, ymin=dist-.25, ymax = dist+.25)) + 
  ...
  ylab(expression(paste("diff. in regr. coefficients (incrasing ", pi(P), ")"))) + 
...
pdf("../figures/fig6.pdf", width = 8.5, height = 4)
