...
for(k in 1:length(n.vec)){
  ...
  for(j in 1:length(dist.vec)){
    ...
    for(i in 1:num.exp){
      ...
      write.table(set.frame, "fig6_data_set.txt", quote = FALSE, sep = "\t")
      write.table(var.frame, "fig6_data_var.txt", quote = FALSE)
    }
  }
}
